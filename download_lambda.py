import os                      # Módulo para interactuar con el sistema (por ejemplo, variables de entorno)
import json                    # Módulo para manejar datos JSON (aunque en este caso no lo usamos)
import re                      # Módulo para expresiones regulares (para extraer datos si fuese necesario)
import requests                # Módulo para hacer peticiones HTTP
import boto3                   # Módulo de AWS para interactuar con servicios como S3
import csv                     # Módulo para escribir archivos CSV
from io import StringIO        # StringIO para crear buffers en memoria para el CSV
...
def download_handler(event, context):
    """
    Descarga las primeras 10 páginas de resultados desde Mitula y guarda el HTML
    completo en un bucket S3.
    """
    ...
    for page in range(1, 11):
        ...
        response = requests.get(url, headers=HEADERS)  # Petición GET con header simulado
        if response.status_code == 200:
            html_content += response.text + "\n"  # Acumulamos el contenido HTML
        else:
    ...
    # Obtenemos el bucket desde variable de entorno o usamos "parcial" por defecto
...
def process_handler(event, context):
    ...
    Extrae los siguientes datos:
      ...
      - Barrio: la ubicación (extraído de <div class="listing-card__location__geo">)
      - NumHabitaciones: número de habitaciones (del atributo content de <p data-test="bedrooms">)
      - NumBanos: número de baños (del atributo content de <p data-test="bathrooms">)
      - mts2: área en metros cuadrados (del atributo content de <p data-test="floor-area">)
    ...
    source_bucket = record["s3"]["bucket"]["name"]  # Bucket fuente
    object_key = record["s3"]["object"]["key"]        # Nombre del archivo HTML en S3
    ...
    # Buscar todos los contenedores de listado usando la clase "listing-card__content"
    listings = soup.find_all("div", class_="listing-card__content")
    if not listings:
        print("No se encontraron listados con la clase 'listing-card__content'")
        return {"status": "error", "message": "No listings found"}

    rows = []  # Lista para almacenar las filas del CSV
    # Se utiliza el nombre del archivo (sin extensión) como fecha de descarga
    fecha_descarga = object_key.replace(".html", "")

    # 4. Iterar sobre cada listado encontrado
    for listing in listings:
        # Extraer el precio: buscamos el <span> con clase "price__actual"
        price_elem = listing.find("span", class_="price__actual")
        valor = price_elem.get_text(strip=True) if price_elem else "N/A"

        # Extraer la ubicación (Barrio): buscamos el <div> con clase "listing-card__location__geo"
        loc_elem = listing.find("div", class_="listing-card__location__geo")
        barrio = loc_elem.get_text(strip=True) if loc_elem else "N/A"

        # Extraer número de habitaciones: <p data-test="bedrooms"> (se obtiene del atributo "content")
        bed_elem = listing.find("p", {"data-test": "bedrooms"})
        num_habitaciones = bed_elem.get("content", "N/A") if bed_elem else "N/A"

        # Extraer número de baños: <p data-test="bathrooms"> (se obtiene del atributo "content")
        bath_elem = listing.find("p", {"data-test": "bathrooms"})
        num_banos = bath_elem.get("content", "N/A") if bath_elem else "N/A"

        # Extraer área en metros cuadrados: <p data-test="floor-area"> (atributo "content")
        area_elem = listing.find("p", {"data-test": "floor-area"})
        mts2 = area_elem.get("content", "N/A") if area_elem else "N/A"

        # Agregar una fila con la información extraída
...
def lambda_handler(event, context):
    ...
    Si el evento contiene registros S3 (clave "Records" con un subobjeto "s3"),
    se ejecuta process_handler (para procesar el HTML y generar el CSV).
    De lo contrario, se ejecuta download_handler (para descargar el HTML).
    """
